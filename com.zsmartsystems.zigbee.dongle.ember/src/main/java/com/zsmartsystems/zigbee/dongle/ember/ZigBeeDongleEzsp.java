...
public class ZigBeeDongleEzsp implements ZigBeeTransportTransmit, ZigBeeTransportFirmwareUpdate, EzspFrameHandler {
    ...
    private static final Set<EmberTrustCenterJoinListener> trustCenterJoinListeners = new HashSet<EmberTrustCenterJoinListener>();
    ...
    private EzspVersionResponse ezspVersionResponse;
    ...
    public void shutdown() {
        ...
        synchronized (isConfiguredSync) {
            if (!isConfigured) {
                logger.debug("[{}]: EZSP Dongle: Shutdown. isConfigured is false. No shutdown necessary.", handlerIdentifier);
                return;
            }
    ...
    public void sendCommand(final int msgTag, final ZigBeeApsFrame apsFrame) {
        if (!isConfigured) {
            logger.debug("[{}]: EZSP Dongle is not configured. Frame not sent: {}", handlerIdentifier, apsFrame);
            return;
        }
    ...
    private ZigBeeStatus setTcJoinMode(TrustCentreJoinMode joinMode) {
        ...
        if (ezspVersion < 8) {
            return (ncp.setPolicy(EzspPolicyId.EZSP_TRUST_CENTER_POLICY,
    ...
    private boolean initialiseEzspProtocol() {
        ...
        if(ezspVersionResponse.getProtocolVersion() != ezspVersion) {
            ...
            ezspVersionResponse = ncp.getVersion();
        }

        StringBuilder builder = new StringBuilder(60);
        builder.append("EZSP Version=");
        builder.append(ezspVersionResponse.getProtocolVersion());
        builder.append(", Stack Type=");
        builder.append(ezspVersionResponse.getStackType());
        builder.append(", Stack Version=");
        for (int cnt = 3; cnt >= 0; cnt--) {
            builder.append((ezspVersionResponse.getStackVersion() >> (cnt * 4)) & 0x0F);
            if (cnt != 0) {
                builder.append('.');
            }
        }
