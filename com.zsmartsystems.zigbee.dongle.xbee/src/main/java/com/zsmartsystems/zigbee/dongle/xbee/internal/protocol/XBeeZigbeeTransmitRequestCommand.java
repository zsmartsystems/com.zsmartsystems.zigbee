...
/**
 * Class to implement the XBee command <b>ZigBee Transmit Request</b>.
 * <p>
 * This frame causes the device to send payload data as an
			RF packet to a specific
 * destination. n For broadcast transmissions,
			set the 64-bit destination address to
 * 0x000000000000FFFF. Address the
			coordinator by either setting the 64-bit address to
 * all 0x00s and the
			16-bit address to 0xFFFE, or setting the 64-bit address to
 * the
			coordinator's 64-bit address and the 16-bit address to 0x0000. n For
			all other
 * transmissions, setting the 16-bit address to the correct
			16-bit address helps improve
 * performance when transmitting to
			multiple destinations. If you do not know a 16-bit
 * address, set this
			field to 0xFFFE (unknown). If successful, the Transmit Status
 * frame
			(0x8B) indicates the discovered 16-bit address. You can set the
			broadcast
 * radius from 0 up to NH. If set to 0, the value of NH
			specifies the broadcast radius
 * (recommended). This parameter is only
			used for broadcast transmissions. You can read
 ...
 */
public class XBeeZigbeeTransmitRequestCommand extends XBeeFrame implements XBeeCommand {
    /**
     * The frame Id
    ...
    /**
     * 0x01 - Disable retries. 0x20 - Enable APS encryption
     ...
     * Setting
					the extended timeout bit causes the stack to set the
     * extended
					transmission timeout for the destination address. See
     * Transmission,
					addressing, and routing. All unused and unsupported bits must
     * be
					set to 0.
    ...
    public String toString() {
        ...
        builder.append("XBeeZigbeeTransmitRequestCommand [frameId=");
